...
function App() {
  ...
  const refreshDB = () => {
    ...
    if (recent && !filtered) {
      ...
      if (!recent) {
        Axios.get(
          `${url.local}/sessions/filter/"${lowBound}"/"${highBound}"`
        )
          .then((res) => {
            setGet(res.data);
            setRequesting(false)
        ...
        Axios.get(
          `${url.local}/sessions/filter/recent/"${lowBound}"/"${highBound}"`
        )
          .then((res) => {
            setGet(res.data);
            setRequesting(false)
  ...
  return (
    <div id="mainContainer">
      <div id="titleContainer">
        {requesting ? 
        <h1>{user}<div id="loading"></div></h1> : 
        <div><h1>{user}'s Lifts</h1><Logout/></div>
        }
        {!loading && !failedReq ? (
